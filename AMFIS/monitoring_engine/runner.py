...
def get_run_bank() -> Callable[[], pd.DataFrame]:
    os.environ.setdefault("MPLBACKEND", "Agg")
    mod = _load_module("bank_pipeline", REPO_ROOT / "src" / "bank_data.py")
    return mod.run_monitoring_for_bank_dataset
...
def run_full_monitoring(
    *,
    ...
    # bank is optional but supported
    try:
        bank_df = get_run_bank()()
    except Exception:
        bank_df = None
