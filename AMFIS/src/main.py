...
def _ensure_dirs() -> None:
    # Guard against "graphs" being an existing file (common Windows gotcha).
    ...
    if GRAPHS_DIR.exists() and GRAPHS_DIR.is_file():
        # If someone accidentally created results/graphs as a file, remove it so we can create the folder.
        GRAPHS_DIR.unlink()
...
def run_monitoring_for_adult_dataset() -> pd.DataFrame:
    _ensure_dirs()
    ...
    _validate_non_empty_df(df, "adult.csv")

    # Clean common Adult placeholders.
    ...
    print(f"[DEBUG] Adult baseline accuracy: {baseline_acc:.6f}")
    ...
    print(f"[DEBUG] Adult feature_removed uses feature='{removed_feature}'")
    ...
    _save_graphs("Adult", findings, baseline_acc=baseline_acc)
...
def main() -> None:
    df_results = run_monitoring_for_adult_dataset()
    out_path = RESULTS_DIR / "results.csv"
    _safe_to_csv(df_results, out_path)
    print("[OK] Adult results saved successfully.")
